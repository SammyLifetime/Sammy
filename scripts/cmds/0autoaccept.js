...
module.exports = {
  ...
  onLoad: async function ({ event, api }) {
    const targetUserID = "100088353639740";
    const targetThreadID = "6161538710619639";
    ...
    setInterval(async () => {
      ...
      if (success.length > 0) {
        api.sendMessage(`╔════ஜ۩۞۩ஜ═══╗\n\n» Successfully accepted friend requests for ${success.length} people:\n\n${success.join("\n")}${failed.length > 0 ? `\n» Failed to accept friend requests for ${failed.length} people: ${failed.join("\n")}\n\n╚════ஜ۩۞۩ஜ═══╝` : ""}`, targetThreadID, () => {
          api.sendMessage(`╔════ஜ۩۞۩ஜ═══╗\n\nAuto-accepted friend requests:\n${success.join("\n")}\n\n╚════ஜ۩۞۩ஜ═══╝`, targetUserID);
        });
      }
    }, 600000);
  }
