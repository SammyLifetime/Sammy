...
module.exports = {
  ...
  onStart: async function ({ event, message, api, usersData, args }) {
    ...
    if (ongoingFights.has(threadID)) {
      return message.send("╔════ஜ۩۞۩ஜ═══╗\n⚔️ A fight is already in progress in this group.\n╚════ஜ۩۞۩ஜ═══╝");
    }

    const mention = Object.keys(event.mentions);

    if (mention.length !== 1) {
      return message.send("╔════ஜ۩۞۩ஜ═══╗\n🤔 Please mention one person to start a fight with.\n╚════ஜ۩۞۩ஜ═══╝");
    }
  ...
  onChat: async function ({ event, message }) {
    ...
    if (attack === "forfeit") {
      ...
      message.send(`╔════ஜ۩۞۩ஜ═══╗\n🏃 ${forfeiter} forfeits the match! ${opponent} wins!\n╚════ஜ۩۞۩ஜ═══╝`);
      endFight(threadID);
      ...
      if (opponent.hp <= 0) {
        ...
        message.send(`╔════ஜ۩۞۩ஜ═══╗\n⏰ Time's up! The game is over. ${winner} wins! ${loser} is defeated.\n╚════ஜ۩۞۩ஜ═══╝`);
        endFight(threadID);
      } else {
        ...
        message.send(`╔════ஜ۩۞۩ஜ═══╗\n🥲 It's ${newCurrentPlayer.name}'s turn currently.\n╚════ஜ۩۞۩ஜ═══╝`);
      }
    } else {
      message.reply(
